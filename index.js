...
function handleMouseMove(event) {
    if ((event.buttons & 1) != 0 && !showingInterstitial) {
        handleSwipe(event.clientX, event.clientY);
    }
}
...
function drawInterstitial() {
    ...
    if (gameOver) {
        ...
        const metrics = context.measureText(gameOverText);
        ...
        context.fillText(gameOverText, textLeft, baseline);
    } else {
        const roundText = "You must clear " + level + " line" + (level > 1 ? "s" : "") + " to pass this level";
        context.fillText(roundText, left + 10, top + 20);
    }
}
