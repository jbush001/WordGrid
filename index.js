...
const FREQ_TABLE = [
    ["Q", 0.0018], ["J", 0.0038], ["X", 0.0068], ["Z", 0.0111], ["W", 0.0205],
    ["V", 0.0307], ["K", 0.0414], ["F", 0.0555], ["Y", 0.0705], ["B", 0.0905],
    ["H", 0.1130], ["M", 0.1404], ["P", 0.1692], ["G", 0.1999], ["U", 0.2345],
    ["C", 0.2724], ["D", 0.3107], ["L", 0.3627], ["O", 0.4235], ["T", 0.4864],
    ["N", 0.5530], ["R", 0.6247], ["A", 0.7010], ["I", 0.7864], ["S", 0.8822],
    ["E", 1.0000],
];
...
function heartBeat() {
    if (gameState == GameState.PLAYING) {
        ...
        } else if (levelEndTime - Date.now() <= 6000) {
...
function startLevel() {
    levelEndTime = Date.now() + ROUND_DURATION_S * 1000;
...
function _testGenerateRandomLetter() {
...
function unpause() {
    if (gameState == GameState.PAUSED) {
        ...
        gameState = GameState.PLAYING;
        levelEndTime += Date.now() - pauseStartTime;
        draw();
    }
}
...
function drawScore() {
    ...
    switch (gameState) {
        case GameState.PAUSED:
            remaining = Math.floor((levelEndTime - pauseStartTime) / 1000);
            break;

        case GameState.PLAYING:
            remaining = Math.floor((levelEndTime - Date.now()) / 1000);
            break;
    }
